...
public:
  ...
  int getQueenCount() { return queens.size(); }
...
public:
  Game(int players) { initCards(players); };
  ...
  void playKnight(int currentPlayer) {
    ...
    int attackPlayerIdx = readInput(msg.str(), [this, currentPlayer](int i) {
  ...
  void PlayTurn(int playerIdx) {
    ...
    std::cout << printCard(playedCard) << std::endl;

    std::visit(Overload{[](Number n) { return; },
                        [this, playerIdx](Knight k) { playKnight(playerIdx); },
                        [](auto c) {
